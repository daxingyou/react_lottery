...
class NavTab extends Component {
    ...
    render() {
        let arr = ["其他","直选","组选","趣味","特殊","定位","不定位","任二","任三","任四"];
        ...
        for(let i in two_level_menu){
            twoRender.push(
                ...
                {two_level_menu[i].map((item, j) =>
                    <a key={j} href="javascript:void(0)" onClick={()=>this.props.TwoLevelClick(item)} className={twoFocus ==  item.method_id ? "cur": ""}>{item.cname}</a>
                )}
            </div>
            )
        }
