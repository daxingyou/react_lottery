...
export let LotteryCommon = ComposeComponent => class extends ComposeComponent {
    ...
    filterLottery(id){
        ...
        if([1,3,4, 8,11,18,24].indexOf(id) !== -1) {
            ...
            path = 'fc3d';
        }else if([12,13,19,20].indexOf(id) !== -1) {
            path = 'k3';
    ...
    getInitData(game_key){
        ...
        Api('c=game&a=initCaiZhong&game_key='+game_key+'&lottery_id=' + this.state.lottery_id, null, (res) => {
            if(res.errno===0){
                let data = res.data;
                ...
                //别删别删别删别删别删别删别删别删别删别删别删别删别删别删别删别删别删别删别删别删别删别删
                let gamePrize = (1 - total_profit+rebate-(sliderConfig.from))/(1-total_profit)*mode.toFixed(3);
                this.setState({
                    gamePrize: gamePrize,
    ...
    transMethods(data){
        ...
        data.map((item,i)=>{
            ...
            item.childs.map((childs,j)=>{
                if(["QSHHZX","ZSHHZX","SXHHZX"].indexOf(childs.name) !== -1){//混合玩法不显示

                }else if(childsObj[childs.method_property]&&childsObj[childs.method_property].length>=0){
        ...
        two_level_menu.map((mdMap,j)=>{
            for(let index in mdMap){
                mdMap[index].map((item,i)=>{
                    if(item.method_id===menu_foucsed[1]){

                        md=mdMap[index][i];
                        menu_foucsed=[j,md.method_id];
                    }
            ...
            //如果没有玩法，则为默认第一个
            if(!md){
                for(let i=0,j=10;i<j;i++){//9种玩法，取第一个被选中
                ...
                menu_foucsed=[0,md.method_id]
            }


        // md=two_level_menu[6][3][0];
        this.setState({
            md:md,
    ...
    TwoLevelClick(item){
        ...
        this.setState({
            menu_foucsed:[one,item.method_id],
    ...
    sliderOnChange(value) {
        ...
        let gamePrize = (1 - total_profit+rebate- (parseFloat(this.state.sliderConfig.to-currentValue).toFixed(3)))/(1-total_profit)*mode;
    ...
    addNumber() {
        ...
        let listData = {
            ...
            'money': this.state.zhushu * mode * this.state.beishu * 2,
        ...
        return (
            <div>
                ...
                <Modal
                    ...
                    footer={null}
                >
                    <div className="play-func-modal" dangerouslySetInnerHTML={{__html: this.props.md.description}}>
