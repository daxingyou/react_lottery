                ...
                <div className="order-detail-wrap">
                    ...
                    {this.props.location.query.page<3 ?<div className="order_deatil">
                        ...
                        <div className="table_toRight">
                            <ul>
                                <li>
                                    <span>总金额：<span className="bbuColor">{data.total_amount}</span></span>
                                    <span className="bbu">中奖金额：<span className="bbuColor">{parseFloat(data.winning_amount).toFixed(3)}</span></span>
                                </li>
                                <li>
                                    <span>返点：<span className="bbuColor">{data.rebate}</span></span>
                                    <span className="bbu">订单盈亏：<span className="bbuColor">{data.show_status=="未开奖"?"未开奖":parseFloat(data.profit_loss.replace(/,/g, "")).toFixed(3)}</span></span>{/*去掉中间的逗号转为数字省略小数点后三位*/}
                                </li>
                            </ul>
                        </div>
                    ...
                    {this.props.location.query.page>=3 ?<div className="order_deatil">
                        ...
                        <div className="table_toRight">
                            <ul>
                                <li>
                                    <span>计划总金额：<span className="bbuColor">￥{parseFloat(data.plan_total_amount).toFixed(3)}</span></span>
                                    <span className="bbu">中奖金额：<span className="bbuColor">￥{ parseFloat(data.total_prize).toFixed(3)}</span></span>
                                </li>
                                <li>
                                    <span>返点：<span className="bbuColor">￥{data.total_rebate}</span></span>
                                    <span className="bbu">当前盈亏：<span className="bbuColor">{data.show_status=="进行中"?"进行中":parseFloat(data.total_profit_loss.replace(/,/g, "")).toFixed(3) }</span></span>
                                </li>
                            </ul>
                        </div>
                        <Table columns={columns} dataSource={tableDate}></Table>
                    ...
                    {/*判斷彩種是否有追號功能   以下四種沒有*/}
                    {data.lottery_id===26 || data.lottery_id===21 || data.lottery_id===9 || data.lottery_id===22 || data.lottery_id===25 ?<div className="xdButton">
                        <Link to={this.fiflerPath(parseInt(this.state.show_status.lottery_id))}>
                            <button className="cx_button">重新下单</button>
                        </Link>
                        <Link to={this.fiflerPath(parseInt(this.state.show_status.lottery_id))}>
