			...
			<div>
						    ...
						    <Link to="user_state">
						       <div className="login-reg_left fl"><i className="icon_touxiang"></i></div>
									 <div className="login-reg_right fl">
										  ...
										  <p className=""><span className="">余额：</span><span className="">{(this.state.balance?parseFloat(this.state.balance):"0.00")}</span><span>元</span></p>
									 </div>
									<div className="login-reg_rr fl"><Icon type="right" style={{ fontSize: 24, color: '#fff',padding:'0.5rem 0 0 0.1rem' }} /></div>
								</Link>
						</div>
					</div>
					<div className="btns">
						<Row className="row">
							...
							<Col className="col" span={8}>
								...
								<div onClick={()=>{this.jumptoWithraw()}}>
				...
				>
					...
					<div className="x-btn-wrapper">
						<span onClick={()=>{this.setState({cardvisible:false})}}>取消</span>
					...
					>
						...
						<div className="x-btn-wrapper">
							<span onClick={()=>{this.setState({psdvisible:false})}}>取消</span>
							<span onClick={()=>{this.handlePsdOk()}}>确定</span>
						</div>
					</Modal>
				</div>
				<Footer />
